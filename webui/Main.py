...
if "local_video_materials" not in st.session_state:
    # 사용자가 마지막으로 저장한 로컬 소재를 기억해 대본만 수정해 다시 생성할 때 소재 목록이 사라지지 않게 합니다.
    st.session_state["local_video_materials"] = []

# 언어 파일 로드
locales = utils.load_locales(i18n_dir)

# 제목과 언어 선택을 포함하는 상단 영역 생성
...
def open_task_folder(task_id):
    try:
        # task_id는 항상 서버가 생성한 UUID여야 합니다. 먼저 형식을 검증해 비정상 값이 경로 결합으로 작업 디렉터리 밖을 가리키거나, 이후 디렉터리를 열 때 플랫폼 shell의 특수 문자 해석을 유발하지 않게 합니다.
        ...
        # UUID 검증을 통과해도 최종 경로가 작업 루트 안에 있는지 다시 확인해, 나중에 호출자가 task_id 출처를 바꿀 때 경로 traversal 위험이 생기지 않게 합니다.
        ...
        with middle_config_panel:
            ...
            if llm_provider == "ollama":
                ...
                with llm_helper:
                    ...
                    if config.is_running_in_container():
                        docker_hint = "\n                            > 컨테이너 환경을 감지했습니다. Base Url이 없으면 기본값으로 사용합니다: `http://host.docker.internal:11434/v1`\n"
                    tips = f"""
                            ##### Ollama 설정 안내
                            - **API Key**: 아무 값이나 입력해도 됩니다. 예: 123
                            - **Base Url**: 일반적으로 http://localhost:11434/v1 입니다.
                                - `MoneyPrinterTurbo`와 `Ollama`가 **같은 머신에 있지 않다면** `Ollama`가 실행 중인 머신의 IP 주소를 입력해야 합니다.
                                - `MoneyPrinterTurbo`를 `Docker`로 배포했다면 다음 값을 권장합니다: `http://host.docker.internal:11434/v1`{docker_hint}
            ...
            if llm_provider == "openai":
                ...
                with llm_helper:
                    tips = """
                            ##### OpenAI 설정 안내
                            > 네트워크 환경에 따라 전역 프록시가 필요할 수 있습니다.
                            - **API Key**: [공식 사이트에서 발급](https://platform.openai.com/api-keys)
                            - **Base Url**: 공식 OpenAI는 비워 둘 수 있습니다. OpenAI 호환 제공업체(예: OpenRouter)를 사용한다면 해당 호환 API 주소를 입력하세요.
                            - **Model Name**: **권한이 있는** 모델을 입력하세요. 호환 제공업체를 사용한다면 해당 플랫폼이 지원하는 모델 ID를 입력하세요.
                            """
            ...
            if llm_provider == "g4f":
                ...
                with llm_helper:
                    tips = """
                            ##### gpt4free 설정 안내
                            > [GitHub 오픈소스 프로젝트](https://github.com/xtekky/gpt4free)，GPT 모델을 무료로 사용할 수 있지만 **안정성이 낮습니다**.
                            - **API Key**: 아무 값이나 입력해도 됩니다. 예: 123
                            - **Base Url**: 비워 두세요.
            ...
            if llm_provider == "deepseek":
                ...
                with llm_helper:
                    tips = """
                            ...
                            - **Base Url**: 고정값: https://api.deepseek.com
                            - **Model Name**: 고정값: deepseek-chat
                            """
            ...
            if llm_provider == "mimo":
                ...
                with llm_helper:
                    tips = """
                            ##### Xiaomi MiMo 설정 안내
                            - **API Key**: [공식 사이트에서 발급](https://platform.xiaomimimo.com/docs/zh-CN/quick-start/first-api-call)
                            - **Base Url**: 고정값: https://api.xiaomimimo.com/v1
                            - **Model Name**: 기본값은 mimo-v2.5-pro이며, 공식 문서에 따라 다른 사용 가능한 모델을 입력할 수도 있습니다.
                            """
            ...
            if llm_provider == "modelscope":
                ...
                with llm_helper:
                    tips = """
                            ##### ModelScope 설정 안내
                            - **API Key**: [공식 사이트에서 발급](https://modelscope.cn/docs/model-service/API-Inference/intro)
                            - **Base Url**: 고정값: https://api-inference.modelscope.cn/v1/
            ...
            if tips and config.ui["language"] in ("zh", "ko"):
                st.warning(
                    "한국어 UI 사용자는 네트워크 환경에 맞는 LLM 제공업체를 선택하세요. DeepSeek, Moonshot, OpenAI 호환 제공업체 등을 사용할 수 있습니다."
                )
                st.info(tips)
...
with middle_panel:
    with st.container(border=True):
        ...
        if params.video_source == "local":
            # Streamlit 파일 형식 검사는 확장자 대소문자에 민감하므로 두 형태를 모두 허용합니다.
    ...
    with st.container(border=True):
        ...
        if selected_tts_server == "siliconflow":
            ...
            # Xiaomi MiMo TTS 프리셋 음색 목록 가져오기
        ...
        # 저장된 음성이 현재 필터링된 목록에 있는지 확인합니다.
        if saved_voice_name in friendly_names:
            saved_voice_name_index = list(friendly_names.keys()).index(saved_voice_name)
        else:
            # 없으면 현재 UI 언어에 맞는 기본 음성을 선택합니다.
            for i, v in enumerate(filtered_voices):
                if v.lower().startswith(st.session_state["ui_language"].lower()):
                    saved_voice_name_index = i
                    break

        # 일치하는 음성이 없으면 첫 번째 음성을 사용합니다.
        ...
        # 선택 가능한 음성이 있을 때만 미리 듣기 버튼을 표시합니다.
        ...
        # V2 버전을 선택했거나 음성이 V2 음성이면 지역과 API Key 입력란을 표시합니다.
        ...
        # SiliconFlow를 선택하면 API Key 입력란과 설명을 표시합니다.
        ...
        # Xiaomi MiMo TTS를 선택하면 MiMo LLM provider의 API Key를 재사용합니다.
        # 사용자가 MiMo로 대본과 음성을 모두 생성할 때 Key를 하나만 관리하면 됩니다.
        ...
        if params.bgm_type == "custom":
            ...
            if custom_bgm_file:
                # 여기서는 os.path.exists를 직접 쓰지 않습니다. 사용자는 output000.mp3처럼 파일명만 입력하는 경우가 많고, 이 값은 서비스 계층에서 resource/songs 디렉터리로 매핑한 뒤 검증해야 합니다. 서비스 계층은 디렉터리와 파일 형식을 제한해 임의 경로 읽기를 막습니다.
...
if start_button:
    ...
    if uploaded_audio_file:
        ...
        # 업로드 파일명은 브라우저에서 오므로 디스크 경로에 직접 붙이지 않습니다. 여기서는 확장자만 보존하고 고정 파일명으로 현재 작업 디렉터리에 저장해 경로 traversal과 특수 문자 문제를 피합니다.
    ...
    if uploaded_files:
        ...
        # 다시 업로드할 때마다 이번에 선택한 소재를 기준으로 삼아 이전 소재가 계속 중복 추가되지 않게 합니다.
        ...
        # 업로드되어 로컬에 저장된 영상 소재를 세션에 기록해 이후 대본만 바꿀 때 바로 재사용합니다.
        st.session_state["local_video_materials"] = persisted_local_materials
    elif params.video_source == "local" and st.session_state["local_video_materials"]:
        # 사용자가 파일을 다시 업로드하지 않으면 마지막으로 디스크에 저장된 로컬 소재 목록을 재사용합니다.
        params.video_materials = []
